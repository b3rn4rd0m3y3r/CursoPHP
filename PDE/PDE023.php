<html>
	...
	<body>
		...
		?>
			<center>
			<form method="post" action="?action=addsave&CodUsr=<?php echo $User; ?>">
				<table>
					<?php
		...
		<?php
			...
			?>
			<center>
			<form method="post" action="?action=editsave&CodUsr=<?php echo $User; ?>">
				<table>
				<?php
			...
			case "dele":
				...
				<a href="?action=delesave&Id=<?php echo $Id ?>&CodUsr=<?php echo $User; ?>">CONFIRMA DELEÇÃO DO REGISTRO <?php echo $Id ?></a>
				</center>
				<?php
				break;
			...
			*/
			case "list":
				...
				while( $row = $comm->fetch() ){
					...
					if( $PERMISSOES->add == "S" ){
						echo "<td align=center><a class=blue href=\"?action=add&CodUsr=" . $User . "\" target=\"NewReg\">&#10009;</a></td>";
						} else {
						echo "<td>&nbsp;</td>";
						}
					if( $PERMISSOES->edit == "S" ){
						echo "<td align=center><a href=\"?action=edit&Id=" . $row["Id"] . "&CodUsr=" . $User . "\" target=\"OldReg\">&#9997;</a></td>";
						} else {
						echo "<td>&nbsp;</td>";
						}
					if( $PERMISSOES->dele == "S" ){
						echo "<td align=center><a href=\"?action=dele&Id=" . $row["Id"] . "&CodUsr=" . $User . "\" target=\"DelReg\">&#10060;</a></td>";
						} else {
						echo "<td>&nbsp;</td>";
						}
					echo "</tr>";
		...
		?>
			<center>
				...
				<form method="post" action="?action=list&CodUsr=<?php echo $User; ?>">
					<div id="container">
					<table class=entry>
						<?php
						foreach ($arr as $chave => $valor) {
				...
				<p>
					<button class="openform" onclick="window.location.href = '?action=add&CodUsr=<?php echo $User; ?>';"><span class=contorno>ABRIR O FORMULÁRIO DE INCLUSÃO DE REGISTRO</span></button>
				</p>
			</center>
		<?php
			}
